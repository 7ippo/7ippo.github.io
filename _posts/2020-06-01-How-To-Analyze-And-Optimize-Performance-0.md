...
![Chrome Timeline瀑布流的bug](https://i.loli.net/2021/04/01/wArj2V7HWkflLIY.png){:data-action="zoom"}{:style="max-width: 85%;"}  
...
> 和引擎层面排查泄漏同理，通过观察反复进出同一场景/打开同一功能引起的内存规模的波动是否正常，一般就能判断游戏内是否存在内存泄漏问题。`H2`项目也因此解决了若干个因为错误使用了`Laya`引擎的动态合批功能而引起的内存暴涨导致的`OOM`问题，修复后将项目内存规模稳定到了Android设备测试环境下400-600MB区间。
