...
description: '讨论浏览器缓存的机制原理，强缓存与协商缓存的区别。Nginx如何设置响应头以及不同浏览器环境如Chrome、Safari、微信浏览器、X5 Webiew下的抓包验证。各种缓存指示如ETag，Last-Modified，Cache-Control，Expires，Pragma对浏览器缓存逻辑的影响。浏览器使用缓存的流程图。'
date: 2019-06-21
author: 周子博
categories: H5
